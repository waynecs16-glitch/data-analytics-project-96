...
lpc_attribution AS (
    ...
    ) AS s_lpc ON s.visitor_id = s_lpc.visitor_id
...
aggregated_performance_daily AS (
    SELECT
        ...
        COUNT(DISTINCT CASE WHEN lpc.lead_id IS NOT NULL THEN lpc.lead_id END) AS leads_count,
...
FROM
    aggregated_performance_daily apd
LEFT JOIN
    ad_costs_daily acd
    ON apd.visit_date = acd.visit_date
